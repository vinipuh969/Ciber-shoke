...
function updateUI(){
  if(currentUser){
    ...
    const modalTheme = document.getElementById('modalThemeStatus');
    const modalAnimation = document.getElementById('modalAnimationStatus');
    const modalCompact = document.getElementById('modalCompactStatus');
    const modalLabels = document.getElementById('modalLabelsStatus');
    
    if(modalTheme) modalTheme.innerHTML = currentUser.theme === 'dark' ? 'Тёмная' : 'Светлая';
    if(modalAnimation) modalAnimation.innerHTML = currentUser.settings?.animations ? 'Вкл' : 'Выкл';
    if(modalCompact) modalCompact.innerHTML = currentUser.settings?.compactMode ? 'Вкл' : 'Выкл';
    if(modalLabels) modalLabels.innerHTML = currentUser.settings?.showPcLabels ? 'Вкл' : 'Выкл';
...
function showSection(section){
  ...
  else if(section === 'deposit'){ document.getElementById('depositBlock').classList.remove('hidden'); if(currentUser) updateUI(); }
}
...
function logout(){ currentUser = null; toggleSidebar(false); showLoginForm(); updateUI(); closeSettingsModal(); showToast('До свидания!'); }
...
function addDeposit(amount) {
  ...
  if(!document.getElementById('depositBlock').classList.contains('hidden')) document.getElementById('depositCurrentBalance').innerHTML = currentUser.balance + ' ₽';
}
...
function toggleTheme(){
  ...
  if(currentUser.settings?.animations) document.querySelector('.customize-item')?.classList.add('effect-pulse');
  setTimeout(() => document.querySelector('.customize-item')?.classList.remove('effect-pulse'), 300);
  updateUI();
  saveData();
}
function toggleAnimationEffect(){ if(currentUser){ currentUser.settings.animations = !currentUser.settings.animations; updateUI(); saveData(); } }
function toggleNotifications(){ if(currentUser){ currentUser.settings.notifications = !currentUser.settings.notifications; updateUI(); saveData(); } }
function toggleDesktopAlerts(){ if(currentUser){ currentUser.settings.desktopAlerts = !currentUser.settings.desktopAlerts; updateUI(); saveData(); } }
function toggleCompactMode(){ if(currentUser){ currentUser.settings.compactMode = !currentUser.settings.compactMode; updateUI(); renderHall(); saveData(); } }
function togglePcLabels(){ if(currentUser){ currentUser.settings.showPcLabels = !currentUser.settings.showPcLabels; updateUI(); renderHall(); saveData(); } }
function resetUserSettings(){ if(currentUser){ currentUser.settings = {...userSettings}; currentUser.theme = 'dark'; updateUI(); renderHall(); saveData(); showToast('Настройки сброшены!'); } }
  ...
  rows.forEach((row, rowIndex) => {
    ...
    for(let i = 0; i < row.count && idx < computers.length; i++){
      ...
      if(pc.status === 'free' && currentUser && currentUser.balance >= 100){
        ...
        card.onclick = () => { if(currentUser?.settings?.desktopAlerts) showToast(devLabel + ' ' + pc.id + ' занят'); };
      }
...
function setupEnterKeyHandler(){
  const le = document.getElementById('loginEmail'), lp = document.getElementById('loginPass');
